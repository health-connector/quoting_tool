class Api::V1::EmployeesController < ApplicationController
  ...
  def start_on_dates
    ...
    minimum_length = EnrollRegistry.resolve "aca_shop_market.open_enrollment.minimum_length_days"
    open_enrollment_end_on_day = EnrollRegistry.resolve "aca_shop_market.open_enrollment.monthly_end_on"
    ...
    start_on =  if current_date.day > minimum_day
                  current_date.beginning_of_month + EnrollRegistry.resolve("aca_shop_market.open_enrollment.maximum_length_months").months
                else
                  current_date.prev_month.beginning_of_month + EnrollRegistry.resolve("aca_shop_market.open_enrollment.maximum_length_months").months
                end

    end_on = current_date - (EnrollRegistry.resolve("aca_shop_market.initial_application.earliest_start_prior_to_effective_on_months").months)
