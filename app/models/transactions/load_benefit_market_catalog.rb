...
module Transactions
  class LoadBenefitMarketCatalog
    ...
    def load_county_zips(input)
      ...
      files = Dir.glob(File.join(Rails.root, "db/seedfiles/plan_xmls/#{input[:state]}/xls_templates/counties", '**',
                                 '*.xlsx'))
    ...
    def load_rating_areas(input)
      ...
      files = Dir.glob(File.join(Rails.root, "db/seedfiles/plan_xmls/#{input[:state]}/xls_templates/rating_areas",
                                 '**', '*.xlsx'))
      parsed_files = parse_files(files)
      parsed_files.each do |file|
    ...
    def load_rating_factors(input)
      ...
      files = Dir.glob(File.join(Rails.root, "db/seedfiles/plan_xmls/#{input[:state]}/xls_templates/rating_factors",
                                 '**', '*.xlsx'))
      parsed_files = parse_files(files)
      parsed_files.each do |file|
    ...
    def load_service_areas(input)
      ...
      files = Dir.glob(File.join(Rails.root, "db/seedfiles/plan_xmls/#{input[:state]}/xls_templates/service_areas",
                                 '**', '*.xlsx'))
      parsed_files = parse_files(files)
      parsed_files.each do |file|
    ...
    def load_plans(input)
      ...
      parsed_files = parse_files(files)
      additional_files = Dir.glob(File.join(Rails.root, "db/seedfiles/plan_xmls/#{input[:state]}/master_xml", '**',
                                            '*.xlsx'))
